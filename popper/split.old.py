...
# @profile
def popper(settings, stats, constrainer, grounder, tester, pos, neg, gen_rules, spec_rules, redundancy_rules, min_size, max_size):
    ...
    for size in range(min_size, max_size+1):
        ...
        while True:
            ...
            if outcome == (Outcome.ALL, Outcome.NONE):
                return (program.clauses, new_gen_rules, new_spec_rules, new_redundacy_rules)
    ...
    return (None, new_gen_rules, new_spec_rules, new_redundacy_rules)
...
def check_old_programs(constrainer, grounder, iteration_progs, min_chunk_prog_size, max_chunk_prog_size, tester, chunk_exs):
    chunk_prog = None
    ...
    for (rule_type, rule) in new_rules:
        for ground_rule in set(ground_rules(grounder, grounder.max_clauses, grounder.max_vars, [rule])):
            if rule_type == 'INCLUSION':
                ...
                new_redundacy_rules.add(ground_rule)
    return (chunk_prog, max_chunk_prog_size, new_spec_rules, new_redundacy_rules)

def process_chunk(stats, settings, tester, constrainer, grounder, neg, gen_rules, spec_rules, redundancy_rules, iteration_progs, num_chunks, chunk_num, chunk_exs, last_ex_prog, min_prog_size, best_prog_size):
    chunk_prog = None
    ...
    if improvement_possible and settings.lazy:
        with stats.duration('check_old_programs'):
            ...
            (better_older_prog, max_chunk_prog_size, hist_specialisation, hist_redundancy) = check_old_programs(constrainer, grounder, iteration_progs, min_chunk_prog_size, max_chunk_prog_size, tester, chunk_exs)

            for ex in chunk_exs:
                spec_rules[ex].update(hist_specialisation)
                redundancy_rules.update(hist_redundancy)
    ...
    if improvement_possible:
        ...
        (new_solution, new_gen_rules, new_spec_rules, new_redundacy_rules) = popper(settings, stats, constrainer, grounder, tester, chunk_exs, neg, gen_rules, chunk_specialisation_rules, chunk_redundancy_rules, min_chunk_prog_size, max_chunk_prog_size)
...
def standard_popper(settings, stats, queue):
    ...
    (new_solution, _new_gen_rules, _new_spec_rules, _new_redundacy_rules) = popper(settings, stats, constrainer, grounder, tester, pos, neg, set(), set(), set(), MIN_SIZE, MAX_SIZE)
    if new_solution != None:
...
def split_search(settings, stats, queue):
    ...
    while True:
        ...
        for i, chunk_exs in enumerate(these_chunks):
            ...
            chunk_prog = process_chunk(stats, settings, tester, constrainer, grounder, neg, gen_rules, spec_rules, redundancy_rules, iteration_progs, num_chunks, i, chunk_exs, last_ex_prog, min_prog_size, best_prog_size)
